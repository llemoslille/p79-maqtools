...
def pesquisar_pedidos_compra_encerrado(pagina=1, registros_por_pagina=100, data_inicial=None, data_final=None):
    payload = {
        "app_key": APP_KEY,
    ...
    try:
        ...
        # Verifica se é erro de "não existem registros" (5113) - não é erro crítico
        if response.status_code == 500 and ("5113" in response.text or "Não existem registros" in response.text or "não existem registros" in response.text):
            print("[INFO] Nenhum registro encontrado para este período - retornando resposta vazia")
            return {"nTotalRegistros": 0, "nTotalPaginas": 0, "pedidos_pesquisa": []}
