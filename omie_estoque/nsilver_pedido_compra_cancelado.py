...
def processar_arquivos_bronze():
    ...
    possiveis_prefixos = [
        "maq_prod_estoque/bronze/pedido_compra_cancelado/",
        "bronze/pedido_compra_cancelado/",
    ]
...
def extrair_data_do_nome_arquivo(nome_arquivo):
    ...
    try:
        ...
        match = re.search(
            r'pedido_compra_cancelado_(\d+)\.parquet', nome_arquivo)
        if match:
...
def main_bronze_para_silver():
    ...
    arquivo_staging_local = "pedidos_compras_cancelado.parquet"
    df_consolidado.to_parquet(arquivo_staging_local, index=False)

    gcs_path_staging = "silver/pedidos_compras/pedidos_compras_cancelado.parquet"
    upload_success = upload_to_gcs(
    ...
    print(f"{'='*60}")
    print(
        f"- Origem: gs://{GCS_BUCKET_NAME}/bronze/pedido_compra_cancelado/ (raiz + processados/)")
    print(
...
def main_validacao():
    ...
    arquivo_staging_gcs = "silver/pedidos_compras/pedidos_compras_cancelado.parquet"
    arquivo_staging_local = "temp_pedidos_compras_cancelado.parquet"
