...
def processar_arquivos_bronze():
    ...
    # Listar todos os arquivos Bronze de pedidos de compra
    bronze_path = "bronze/pedido_compra/"
    arquivos_bronze = listar_arquivos_gcs(GCS_BUCKET_NAME, bronze_path)

    if not arquivos_bronze:
        print("[ERRO] Nenhum arquivo Bronze encontrado!")
        return None

    print(f"[INFO] Encontrados {len(arquivos_bronze)} arquivos Bronze")
    ...
    for i, arquivo_gcs in enumerate(arquivos_parquet, 1):
        ...
        caminho_relativo = arquivo_gcs.replace(
            bronze_path, '') if bronze_path else arquivo_gcs
        print(
        ...
        if baixar_arquivo_gcs(GCS_BUCKET_NAME, arquivo_gcs, arquivo_local):
            try:
                ...
                #     pasta_destino = f"bronze/pedido_compra/processados/{ano}/{mes}/"
                ...
                #         print(f"  [OK] Arquivo movido com sucesso")
                #     else:
                #         print(f"  [ERRO] ao mover arquivo")
                # elif not eh_arquivo_raiz:
                #     print(f"  [INFO] Arquivo ja em pasta processados/ (nao sera movido)")
...
def renomear_colunas(df):
    ...
    # Dicionário de mapeamento (original -> novo)
    mapeamento = {
...
def extrair_data_do_nome_arquivo(nome_arquivo):
    ...
    try:
        ...
        match = re.search(r'pedido_compra_(\d+)\.parquet', nome_arquivo)
        if match:
...
def main_bronze_para_silver():
    ...
    arquivo_staging_local = "pedidos_compras_stg.parquet"
    df_consolidado.to_parquet(arquivo_staging_local, index=False)

    gcs_path_staging = "silver/pedidos_compras/pedidos_compras_stg.parquet"
    upload_success = upload_to_gcs(
    ...
    print(f"{'='*60}")
    print(
        f"- Origem: gs://{GCS_BUCKET_NAME}/bronze/pedido_compra/ (raiz + processados/)")
    print(
        f"- Movimentacao de arquivos: DESABILITADA (arquivos permanecem no local original)")
    print(f"- Staging gerado: gs://{GCS_BUCKET_NAME}/{gcs_path_staging}")
    print(f"- Registros processados: {len(df_consolidado)}")
    print(f"[INFO] [OK] Arquivo final será gerado pelo nsilver_pedido_compra.py")
...
def main_validacao():
    ...
    arquivo_staging_local = "temp_pedidos_compras_stg.parquet"
    ...
    print(f"- Arquivo destino: gs://{GCS_BUCKET_NAME}/{gcs_path_final}")
    print(f"- Registros processados: {len(df_consolidado)}")
    print(f"- Colunas finais: {len(df_consolidado.columns)}")

    print(f"\n[OK] Validação concluída com sucesso!")
    print(
        f"[INFO] Arquivo final disponível em: gs://{GCS_BUCKET_NAME}/{gcs_path_final}")
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--concatenar":
        # Modo concatenação: une os dois arquivos Silver
        print("[MODO] Concatenação de arquivos Silver")
        concatenar_arquivos_silver()
    elif len(sys.argv) > 1 and sys.argv[1] == "--validacao":
        # Modo validação: processa staging para final
        print("[MODO] Processamento de validação (staging -> final)")
        main_validacao()
    else:
        # Modo padrão: processamento direto Bronze -> Silver
