...
from configuracoes.variaveis import app_key, app_secret, base_url, endpoint_clientes, version
...
def carregar_config() -> dict:
    """Carrega as configurações do projeto (mesmo padrão do omie_vendas)."""
    config_path = Path(__file__).parent / "configuracoes" / "config.yaml"

    if not config_path.exists():
        logger.warning("Arquivo config.yaml não encontrado, usando configurações padrão")
        return {
            "bucket-raw": (os.getenv("GCS_BUCKET_RAW") or "").strip(),
            "credentials-path": (os.getenv("GCS_CREDENTIALS_PATH") or "").strip(),
        }
...
def salvar_gcs(
    ...
    if not bucket_name:
        raise ValueError(
            "Bucket RAW não configurado. Verifique omie_vendas/configuracoes/config.yaml (bucket-raw) "
            "ou variável GCS_BUCKET_RAW"
        )
    ...
    # Caminho fixo dentro do bucket RAW: clientes/clientes.parquet
    nome_arquivo = "clientes.parquet"
    blob_path = f"{subpasta}/{nome_arquivo}"
