...
def salvar_gcs_silver(df: pd.DataFrame, subpasta: str = "clientes"):
    ...
    bucket_name = (config.get("bucket-silver") or "").strip()
    
    if not bucket_name:
        raise ValueError(
            "Bucket SILVER não configurado. Verifique omie_vendas/configuracoes/config.yaml (bucket-silver)"
        )
...
def listar_arquivos_gcs(subpasta: str = "clientes"):
    ...
    # Estrutura dentro do bucket RAW: subpasta/
    caminho = f"{subpasta}/"
